...
// 反推最后 2 字节，使得 crc16(full)==0，算法与您 Python 版一致
static std::pair<uint8_t,uint8_t> crc16_tail_bytes(const uint8_t* prefix, size_t n) {
    // 预构逆表：val(16bit) -> 某个 idx(0..255) 使得 CRC_TABLE[idx]==val
    static std::array<int, 65536> inv{};      // -1 表示无
    static bool built = false;
    if (!built) {
        inv.fill(-1);
        for (int i = 0; i < 256; ++i) inv[CRC_TABLE[i]] = i;
        built = true;
    }
    uint16_t s = 0;
    for (size_t i = 0; i < n; ++i) {
...
static inline void put_be16(std::vector<uint8_t>& o, uint16_t v){ o.push_back((v>>8)&0xFF); o.push_back(v&0xFF); }
static inline void put_be24(std::vector<uint8_t>& o, uint32_t v){ o.push_back((v>>16)&0xFF); o.push_back((v>>8)&0xFF); o.push_back(v&0xFF); }
static inline void put_be32(std::vector<uint8_t>& o, uint32_t v){ o.push_back((v>>24)&0xFF); o.push_back((v>>16)&0xFF); o.push_back((v>>8)&0xFF); o.push_back(v&0xFF); }
...
struct HCAHeader {
    ...
    // 可选
    bool has_vbr=false; uint16_t vbr_max_frame_size=0, vbr_noise_level=0;
    int  ath_type=-1; bool has_ath=false;
    bool loop_flag=false; uint32_t loop_start_frame=0, loop_end_frame=0; uint16_t loop_start_delay=0, loop_end_padding=0;
    bool has_ciph=false; uint16_t ciph_type=0;
    bool has_rva=false;  float rva_volume=0.f;
    bool has_comm=false; std::string comment;

    size_t data_offset=0;
    // 记录原始 chunk 出现情况
    bool used_comp=true; // 否则就是 dec
};

// ---------- 解析 HCA 头（只读我们需要的字段） ----------
static HCAHeader parse_hca_header(const std::vector<uint8_t>& buf){
    if (buf.size() < 8) throw std::runtime_error("Too small");
    const uint8_t* p = buf.data();
    uint32_t tag = be32(p); p += 4;
    if ( (tag & HCA_MASK) != 0x48434100 ) throw std::runtime_error("Not HCA");
    HCAHeader h{};
    h.version = be16(p); p += 2;
    h.header_size = be16(p); p += 2;
    if (buf.size() < h.header_size) throw std::runtime_error("Short header");
    if (crc16_sum(buf.data(), h.header_size) != 0) throw std::runtime_error("Header CRC failed");

    // fmt
    tag = be32(p);
    if ( (tag & HCA_MASK) != 0x666D7400 ) throw std::runtime_error("Missing fmt");
    p += 4;
    h.channels = *p++;
    h.sample_rate = be24(p); p += 3;
    h.frame_count = be32(p); p += 4;
    h.encoder_delay = be16(p); p += 2;
    h.encoder_padding = be16(p); p += 2;
    ...
    if ( (tag & HCA_MASK) == 0x636F6D70 ){ // comp
        p += 4;
        h.used_comp = true;
        h.frame_size = be16(p); p+=2;
        h.min_resolution = *p++; h.max_resolution = *p++;
        h.track_count = *p++; h.channel_config = *p++;
        h.total_band_count = *p++; h.base_band_count = *p++;
        h.stereo_band_count = *p++; h.bands_per_hfr_group = *p++;
        h.ms_stereo = *p++; p++; // reserved
    ...
    if ( (tag & HCA_MASK) == 0x76627200 ){
        p += 4;
        h.has_vbr = true;
        h.vbr_max_frame_size = be16(p); p+=2;
        h.vbr_noise_level    = be16(p); p+=2;
        tag = be32(p);
    }
    ...
    if ( (tag & HCA_MASK) == 0x6C6F6F70 ){
        p += 4;
        h.loop_flag = true;
        h.loop_start_frame = be32(p); p+=4;
        h.loop_end_frame   = be32(p); p+=4;
        h.loop_start_delay = be16(p); p+=2;
        h.loop_end_padding = be16(p); p+=2;
        tag = be32(p);
    }
    ...
    h.data_offset = h.header_size; // 帧数据紧跟其后
    return h;
}
...
static std::vector<uint8_t> build_plain_header_bytes(const HCAHeader& s){
    ...
    if (s.has_rva){
        ...
        uint32_t u; std::memcpy(&u, &s.rva_volume, 4);
        put_be32(chunks, u);
    }
...
static std::array<uint8_t,256> cipher_init1(){
    std::array<uint8_t,256> t{};
    t[0]=0; t[0xFF]=0xFF;
    int mul=13, add=11; int v=0;
    for (int i=1;i<255;++i){
        v=(v*mul+add)&0xFF;
        if (v==0 || v==0xFF) v=(v*mul+add)&0xFF;
        t[i]=(uint8_t)v;
    }
    return t;
}
static void cipher_init56_create_table(uint8_t key_byte, uint8_t out16[16]){
    ...
    for (int i=0;i<16;++i){ key = (uint8_t)((key*mul + add) & 0xF); out16[i]=key; }
}
static std::array<uint8_t,256> cipher_init56(uint64_t keycode_low56){
    if (keycode_low56 != 0) keycode_low56 = (keycode_low56 - 1) & ((1ULL<<56)-1);
    uint8_t kc[8]{}; // 仅用 7 字节
    for (int i=0;i<7;++i){ kc[i] = (uint8_t)(keycode_low56 & 0xFF); keycode_low56 >>= 8; }

    uint8_t seed[16]{};
    seed[0x00]=kc[1]; seed[0x01]=kc[1]^kc[6]; seed[0x02]=kc[2]^kc[3]; seed[0x03]=kc[2];
    seed[0x04]=kc[2]^kc[1]; seed[0x05]=kc[3]^kc[4]; seed[0x06]=kc[3]; seed[0x07]=kc[3]^kc[2];
    seed[0x08]=kc[4]^kc[5]; seed[0x09]=kc[4]; seed[0x0A]=kc[4]^kc[3]; seed[0x0B]=kc[5]^kc[6];
    seed[0x0C]=kc[5]; seed[0x0D]=kc[5]^kc[4]; seed[0x0E]=kc[6]^kc[1]; seed[0x0F]=kc[6];

    uint8_t base_r[16]; cipher_init56_create_table(kc[0], base_r);
    uint8_t base[256]{};
    for (int r=0;r<16;++r){
        uint8_t base_c[16]; cipher_init56_create_table(seed[r], base_c);
        uint8_t nb = (uint8_t)((base_r[r] & 0xF) << 4);
        for (int c=0;c<16;++c) base[r*16+c] = nb | (base_c[c] & 0xF);
    }

    std::array<uint8_t,256> t{}; t[0]=0; t[0xFF]=0xFF;
    int pos=1, x=0;
    for (int i=0;i<256;++i){
        x = (x + 17) & 0xFF;
        uint8_t bx = base[x];
        if (bx!=0 && bx!=0xFF){
            if (pos < 0xFF){ t[pos]=bx; ++pos; }
        }
    }
    return t;
}
static std::array<uint8_t,256> build_cipher_table(uint16_t ciph_type, uint64_t key_low56){
    if (ciph_type==56 && key_low56==0) ciph_type=0;
    if (ciph_type==0)  return cipher_init0();
    if (ciph_type==1)  return cipher_init1();
    if (ciph_type==56) return cipher_init56(key_low56);
...
// ---------- 主流程：解密为 ciph=0 的新 HCA ----------
static std::vector<uint8_t> decrypt_hca_to_plain_bytes(const std::vector<uint8_t>& in_bytes,
                                                       uint64_t mainkey,
                                                       py::object py_subkey /* Optional[int] or None */)
{
    // 解析原头
    ...
    for (uint32_t i=0; i<H.frame_count; ++i){
        if (off + fsz > in_bytes.size()) break; // 容错截断
        std::vector<uint8_t> frame(in_bytes.begin()+off, in_bytes.begin()+off+fsz);

        // 解密
        for (size_t j=0;j<frame.size();++j) frame[j] = table[frame[j]];
...
// ---------- pybind11 绑定 ----------
PYBIND11_MODULE(hcadecrypt, m) { // 模块名：hcadecrypt
    ...
    m.def("decrypt",
          [](py::bytes data, uint64_t mainkey, py::object subkey)->py::bytes{
              // 取出输入 bytes 的拷贝（py::bytes 语义）
              std::string in = data; // bytes -> std::string（字节安全）
          ...
          py::arg("data"), py::arg("mainkey"), py::arg("subkey") = py::none(),
          R"pbdoc(
